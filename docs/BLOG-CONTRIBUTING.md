...
- **실제 사이트에서 미공개 미리보기** → 글이 **머지·배포되면**, `draft: true`인 상태에서도
  **`https://labs.plateer.com/blog/<slug>` URL을 직접 열면** 실제 사이트 모습으로 확인됩니다.
  - 이 초안 페이지는 **목록·검색·사이트맵엔 뜨지 않고**(URL 아는 사람만), 상단에 "초안 미리보기" 배너 + `noindex`가 붙습니다.
  - 확인 후 좋으면 프론트매터를 `draft: false`로 바꿔 다시 올리면 → 목록·검색에 정식 노출(발행).
  - ⚠️ 운영은 `main`에 머지된 것만 빌드합니다. **머지 전(PR/fork) 상태는 운영 URL로 안 보이니**, 그 단계에선 위 `/admin` 미리보기를 쓰세요.
